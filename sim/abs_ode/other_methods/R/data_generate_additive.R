...
poly3abs <- function(x) return(c(abs(x),x^2,x^3))
...
generate.ode <- function(p=10, t1=0, t2=20, by=0.5,
  ...
  ode.model <- function(t, state, parameters) {
    a0 <- parameters[[1]]
    a1 <- parameters[[2]]
    dstate1 <- 1.2*state[1]+0.3*state[1]^2-0.6*state[1]^3 +
      0.1*state[2]+ 0.2*state[2]^2 + 0.2*state[2]^3
    dstate2 <- 0.4 - 2*state[1]+0.4*state[1]^3 +
      0.5*state[2]+0.2*state[2]^2-0.3*state[2]^3
    dstate3 <- -0.2+ 0.3*abs(state[4])+ 0.4*state[4]^2+ 0.1*state[4]^3
    dstate4 <- -0.2+ 0.2*abs(state[3])-0.1*state[3]^2 -0.2*state[3]^3
    dstate5 <- 0.05 + 0.1*state[6]-0.8*state[6]^3
    dstate6 <- -0.05+0.5*state[5]^3
    dstate <- c(dstate1,dstate2,dstate3,dstate4,dstate5,dstate6,a0p)
    return(list(dstate))
  }
...
generate.ydata <- function(p=10, t1=0, t2=20, by=0.5, R = 1,
  ...
  true_fun[[2*2-1]][[2*2]] <- local({
    a21 <- a1[(6*2-2):(6*2),2*2-1]
    function(x) {sapply(x,function(xx){sum(a21*poly3abs(xx))})}
  })
  true_fun[[2*2]][[2*2-1]] <- local({
    a12 <- a1[(6*2-5):(6*2-3),2*2]
    function(x) {sapply(x,function(xx){sum(a12*poly3abs(xx))})}
  })
