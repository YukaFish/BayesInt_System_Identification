...
generate.ode <- function(p=10, t1=0, t2=20, by=0.5,
  ...
  if (is.null(state_init)) {
    state1 <- rep(c(5,4,7.5,5.5,8),each=2)
    state2 <- rnorm(p2)
    state  <- (rep(c(5,4,7.5,5.5,8),each=2) - shifts)/scales
  } else {
  ...
  eta <- deSolve::ode(state, times, ode.model, pair)[,-1]
  d.eta <- get.d.eta(eta)
  etadense <- deSolve::ode(state, td, ode.model, pair)[,-1]
...
generate.ydata <- function(p=10, t1=0, t2=20, by=0.5, R = 1,
  ...
  G4 <- matrix(0,4,4)
  graph <- as.matrix(Matrix::bdiag(list(G1,G1,G1,G1,G1)))
