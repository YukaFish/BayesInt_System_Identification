...
new_fit_lst <- Sys.glob(paste('sim.result.', 
                              obs_choice$obs, 'obs.', 
                              snr, 'snr.*.rds', sep=""))
time_lst <- Sys.glob(paste('time.result.', 
                           obs_choice$obs, 'obs.', 
                           snr, 'snr.*.rds', sep=""))
...
# define the true graph
true_matrix <- matrix(F,nrow=num_sigma,ncol=num_sigma)
true_matrix[1,1] = true_matrix[1,2] = true_matrix[2,1] = true_matrix[2,2] = T
true_matrix[4,3] = true_matrix[3,4] = true_matrix[6,5] = true_matrix[5,6] = T
...
for (i in 1:length(time_lst)) {
  runtime_lst <- c(runtime_lst, readRDS(time_lst[i]))
}
...
result <- list(cov_prob_traj_lst = cov_prob_traj_lst, 
               ...
               tpr=tpr, fpr=fpr, runtime_lst = runtime_lst)
